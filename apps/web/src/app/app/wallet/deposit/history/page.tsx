// Path: apps/web/src/app/app/wallet/deposit/history/page.tsx
"use client";
...
import { useDepositHistory } from "@/lib/wallet/queries";
import { backendMessage, formatFecFromMilli } from "@/lib/wallet/ui";
import type { DepositHistoryItem } from "@/lib/wallet/api";
...
export default function DepositHistoryPage() {
  ...
  const deposits = useDepositHistory(50, gateOk);
      ...
      <div className="mx-auto w-full max-w-md space-y-4">
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-[#6B7C99] dark:text-[#8FA0BC]">
            Your last 50 client deposit records.
          </p>
        </div>
        ...
        {gateOk ? (
          <div className="rounded-2xl border border-[#C5D5EE] dark:border-[#2D3F55] bg-white dark:bg-[#1E2A3A] p-4 shadow-[0_4px_24px_rgba(91,143,204,0.12)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
            {deposits.isLoading ? (
              ...
              <div className="text-sm text-[#D9534F] dark:text-red-300">
                {backendMessage(deposits.error) ?? "Failed to load deposits."}
              </div>
            ) : (deposits.data?.items?.length ?? 0) === 0 ? (
              <div className="text-sm text-[#6B7C99] dark:text-[#8FA0BC]">No deposits yet.</div>
            ) : (
              <div className="space-y-2">
                {deposits.data!.items.map((d: DepositHistoryItem) => (
                  <div
                    key={d.id}
                  ...
                  >
                    ...
                    <div className="shrink-0 text-right">
                      <div className="font-semibold text-[#1A2B4A] dark:text-[#E8F0FA]">
                        {formatFecFromMilli(Number(d.amountMilliFec ?? 0))}
                      </div>
                      <div className="text-xs text-[#6B7C99] dark:text-[#8FA0BC]">
                        {Number(d.amountKobo ?? 0) / 100} NGN
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="rounded-2xl border border-[#C5D5EE] dark:border-[#2D3F55] bg-white dark:bg-[#1E2A3A] p-4 shadow-[0_4px_24px_rgba(91,143,204,0.12)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
            <div className="text-sm text-[#6B7C99] dark:text-[#8FA0BC]">
              {!token ? (
                <>
                  You need to be logged in to view deposit history.{" "}
                  <Link
                    href="/login"
                ...
                <>
                  Deposit history is only available in CLIENT mode.{" "}
                  <Link
                    href="/app/profile"
                    className="underline text-[#5B8FCC] dark:text-[#7AAEE0]"
                  >
                ...
                <>
                  Your account must be verified to access deposits.{" "}
                  <Link
