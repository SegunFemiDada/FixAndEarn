...
export async function listPendingVerifications(params?: {
  ...
  const res = await adminApi.get<PendingVerificationRow[]>(
...
export async function getVerificationDetail(
  ...
  const res = await adminApi.get<AdminVerificationDetail>(
...
export async function submitNinVerificationDecision(
  id: string,
  payload: NinVerificationDecisionPayload,
): Promise<NinVerificationDecisionResponse> {
  const res = await adminApi.post<NinVerificationDecisionResponse>(
    `/admin/verification/${id}/nin-decision`,
    payload,
  );
