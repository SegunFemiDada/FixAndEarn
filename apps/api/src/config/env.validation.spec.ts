...
function createValidConfig(): Record<string, unknown> {
  return {
    ...
    DATABASE_URL: "postgresql://postgres:postgres@localhost:5432/fixandearn",
...
describe("validateEnvironment", () => {
  it("accepts a valid testing configuration", () => {
  ...
  it("accepts the Monnify sandbox URL", () => {
    const config = createValidConfig();
  ...
  it("accepts the Monnify production URL", () => {
    const config = createValidConfig();
  ...
  it("rejects an invalid Monnify URL", () => {
    const config = createValidConfig();

    config.MONNIFY_BASE_URL = "not-a-url";
  ...
  it("rejects an invalid face match threshold", () => {
    const config = createValidConfig();

    config.FACE_MATCH_DUPLICATE_THRESHOLD = "101";
  ...
  it("accepts a valid 32-byte bank encryption key", () => {
    const config = createValidConfig();
  ...
  it("rejects a bank encryption key with the wrong size", () => {
    const config = createValidConfig();
