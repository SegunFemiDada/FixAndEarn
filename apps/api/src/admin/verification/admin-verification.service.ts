...
export class AdminVerificationService {
  ...
  async decide(args: {
    ...
    if (rec.status !== "PENDING") {
      throw new ForbiddenException("VERIFICATION_NOT_PENDING");
    }

    if (
      args.action === "APPROVE" &&
      rec.ninVerificationStatus !== "VERIFIED"
    ) {
      throw new BadRequestException("NIN_VERIFICATION_REQUIRED");
    }
    ...
    if (args.action === "REQUEST_REUPLOAD") {
      reason = `REQUEST_REUPLOAD: ${cleanReason}${
        cleanReuploadFields.length
          ? ` | FIELDS: ${cleanReuploadFields.join(", ")}`
          : ""
      }`;
    }
    ...
    const previousRekognitionFaceId =
      rec.rekognitionFaceId ?? null;
    ...
    if (args.action === "APPROVE") {
      ...
      // Re-check at approval time so a newer approved identity
      // cannot slip through after the applicant originally submitted.
  ...
  async decideNin(args: {
    ...
    if (!note) {
      throw new BadRequestException(
        "NIN_VERIFICATION_NOTE_REQUIRED",
      );
    }

    const ninVerificationStatus =
      args.action === "VERIFY" ? "VERIFIED" : "FAILED";

    const updated = await this.repo.decideNin({
      id: args.verificationId,
    ...
    await this.audit.log({
      actorAdminId: args.adminId,
      action: "NIN_VERIFICATION_DECISION",
      description: `NIN verification ${args.action}`,
      metadata: {
        verificationId: args.verificationId,
    ...
    return {
      ok: true,
      status: updated.ninVerificationStatus,
    };
  }
}