...
const MAX_VERIFICATION_FILE_SIZE =
  2 * 1024 * 1024;
...
export class VerificationController {
  ...
  async getMyVerification(
    ...
    return this.verification.getMine(
      user.userId,
    );
  }
  ...
  async submit(
    ...
    try {
      ...
      const record =
        await this.verification.submit(
          user.userId,
          {
            ...dto,
            ninImagePath,
      ...
      cleanupResults.forEach(
        (
          ...
          if (
            ...
            this.logger.error(
              `Failed to clean up verification file ${uploadedPaths[index]}: ${message}`,
              stack,
            );
          }
        },
